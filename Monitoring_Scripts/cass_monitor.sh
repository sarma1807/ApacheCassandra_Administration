...
then
  ...
  echo -e "<br><br>Report generated on `hostname -s` @ `date +'%Y-%m-%d %H:%M:%S'` \n\n" >> $CASS_STATUS_EMAIL
  ...
  then
    if [[ "${ALL_NODES_DOWN}" = "Y" ]]
    then
      echo "ALL NODES might be down. ${NODES}" | mail $TO_PAGER_LIST
    else
      echo "${COUNT_DN_NODES} node(s) in ${CASS_CLUSTER_NAME} might be down." | mail $TO_PAGER_LIST
    fi
